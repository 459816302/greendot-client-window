...
namespace ShadowSocks.View
{
    public partial class Main : MaterialForm
    {
        ...
        private int userCheckDuration = 1000 * 30;
        ...
        public async void userCheckAction(Object source, EventArgs e)
        {
            //定时器处理函数
            try
            {
                await Request.instance().Post(Constants.USER_INFO_API);
                JObject res = Request.instance().PostResult;

                if (res.ok())
                {
                    var user = User.instance();
                    var preUserPhone = user.phone;
                    var preUserType = user.type;
                    var preUserExpireTime = user.expiretime;
                    var preUserAvatar = user.avatar;

                    var thisResContent = res.content();
                    var thisAvatar = (string)thisResContent["avatar"]; 
                    var thisExpiretime = (string)thisResContent["serviceexpiredate"];
                    var thisPhone = (string)thisResContent["phone"];
                    var thisType = Convert.ToInt32(thisResContent["type"]);

                    user.setUser(thisResContent);

                    if (user.expired || !preUserPhone.Equals(thisPhone) || preUserType != thisType || !preUserExpireTime.Equals(thisExpiretime) || !preUserAvatar.Equals(thisAvatar)) {
        ...
        private async void subServerConnectCount(int serverId)
        {
            ...
            try
            {
                await Request.instance().Post(Constants.SERVER_DISCONNECT_API, postData);
            }
            catch { }
        }

        private async void addServerConnectCount(int serverId)
        {
            var postData = new List<KeyValuePair<string, string>>();
            var postServerId = DES.DESEnCode(serverId.ToString());
            postData.Add(new KeyValuePair<string, string>("sid", postServerId));
            try
            {
                await Request.instance().Post(Constants.SERVER_CONNECT_API, postData);
            }
            catch(Exception ex){
        ...
        private async void btnLogout_Click(object sender, EventArgs e)
        {
            ...
            await Request.instance().Post(Constants.USER_LOGOUT_API, logoutData);
